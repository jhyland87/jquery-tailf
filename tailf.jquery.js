class TailF {
	 ...
	 * @todo 	Add the 'starting' parameter (equiv to tail -f -n N), or at least the ability to update the last line? (EG: The output of echo -n "Hello "; echo "World" would be on one line)
	 */
	...
	init(){
		...
		this._loop = setInterval(() => {
			...
			// Make sure theres some content to pass to the parser
			if( attained.length === 0 ){
				console.debug('No attained data')
				return
			}
			...
			if( attainedLinesArr.length !== Object.keys( this._buffer ).length ){
				...
				for( var i = Object.keys( this._buffer ).length; i < attainedLinesArr.length; i++ ){ 
					newLines[ i+1 ] = this._buffer[ i+1 ]  = attainedLinesArr[ i ] 
